...
<template>
  <section id="portafolio" class="py-20">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
      <a v-for="proj in projects" 
         :key="proj.name" 
         :href="proj.link" 
         target="_blank"
         class="inbio-card group p-6 rounded-[20px] bg-[#212428] transition-all duration-500 hover:-translate-y-3 block no-underline outline-none"
      >
        ...
        <div class="flex justify-between items-center mb-4">
          <span class="text-accent text-[10px] uppercase font-bold tracking-[0.2em]">{{ proj.category }}</span>
          <div class="flex gap-2">
            <span v-for="tag in proj.tags" :key="tag" class="text-[9px] bg-emerald-950/50 text-emerald-400 border border-emerald-500/20 px-2 py-1 rounded">
              {{ tag }}
            </span>
          </div>
        </div>

        <h3 class="text-2xl title-main text-[#c4cfde] group-hover:text-accent transition-colors mb-3 flex items-center justify-between">
          {{ proj.name }}
          <i class="ri-arrow-right-up-line opacity-0 group-hover:opacity-100 transition-all transform group-hover:-translate-y-1"></i>
        </h3>
        
        <p class="text-[#878e99] text-sm leading-relaxed mb-6 text-justify">
          {{ proj.description }}
        </p>
...
<style scoped>
/* Estilo InBio: Sombras neumórficas profundas */
.inbio-card {
...
.inbio-card:hover {
  background: linear-gradient(145deg, #212428, #1e2024);
  box-shadow: 15px 15px 30px #191b1e, -15px -15px 30px #292d32;
}
...
/* Evitar que el texto herede estilos de enlace por defecto */
p, span, h3 {
  text-decoration: none !important;
}
</style>